...
class Approval:
    approval_id:str; approver:str; proof_ref:str; requested_action:str; target:str; scope:tuple[str,...]; risk_class:str; consequences:tuple[str,...]; conditions:tuple[str,...]; valid_from:str; expires_at:str|None; decision:ApprovalDecision; revocation_state:RevocationState; task_ref:str; action_request_ref:str; audit_ref:str; step_ref:str|None=None
    def body(self):
        d=asdict(self); d["decision"]=str(self.decision); d["revocation_state"]=str(self.revocation_state); return d

class ApprovalRegistry:
    """Append-only Approval history. Consuming approval creates a new versioned event, never edits the original body."""
    def __init__(self, connection:sqlite3.Connection):
        ...
        CREATE TABLE IF NOT EXISTS approvals(
          ...
          action_request_ref TEXT NOT NULL,
    ...
    def get(self, approval_id, version=None):
        ...
        return dict(row)
    def approved_for(self, *, task_ref, action_request_ref, operation, target, required_scope, risk_class, now):
        rows=self.connection.execute("SELECT * FROM approvals WHERE task_ref=? AND action_request_ref=? ORDER BY sequence DESC",(task_ref,action_request_ref)).fetchall()
        for row in rows:
            ...
            return dict(row)
        return None
    def consume(self, approval_id, *, task_ref, action_request_ref):
