...
def prepare_multistep(
    ...
    *,
    ...
    for key in order:
        ...
        if not matches:
            raise MultiStepRejected(
                f"no active capability matches step={key!r}, operation={req.operation!r}, domain={req.domain!r}"
            )
        try:
            match = select_capability(matches, kernel._provider_health_for)
        except CapabilitySelectionRejected as exc:
            raise MultiStepRejected(f"step {key!r} has no routable provider: {exc}") from exc
        cap = match.capability
        if not cap.provider_ref:
            raise MultiStepRejected(f"step {key!r} capability has no provider/adapter binding")
        resolved[key] = cap
    ...
    conclusion = {
        key: {
            "capability_ref": resolved[key].capability_id,
            "version": resolved[key].version,
            "provider_ref": resolved[key].provider_ref,
        }
        for key in order
    }
    decision = kernel.mind.decide(
        goal_ref=goal.goal_id,
        question="Which registered capabilities should execute the ordered outcome steps?",
        conclusion=conclusion,
        rationale="Selected provider-health-aware capabilities for every dependency-ordered step without granting authority.",
        authority_basis=authority_basis,
    ...
    for key in order:
        req = by_key[key]
        cap = resolved[key]
        dependencies = tuple(step_ids[dependency] for dependency in req.dependencies)
        created[key] = kernel.nervous.create_step(
            ...
            state=StepState.READY if not dependencies else StepState.PLANNED,
        )
    plan = kernel.mind.plan(
        goal_ref=goal.goal_id,
        decision_ref=decision.decision_id,
        step_refs=tuple(step_ids[key] for key in order),
        ...
        reason="Execute dependency-ordered canonical Steps with authority evaluated independently per effect.",
        plan_id=plan_id,
    )
    entries = [
        ContextEntry(
            ...
            "CONFIRMED",
            intent.sensitivity,
            "The current request defines the multi-step outcome and scope.",
            project_boundary,
        )
    ]
