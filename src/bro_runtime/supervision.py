...
class TaskSupervisor:
    ...
    def open_flow(
        ...
        *,
        ...
        """Create the Task, bind its canonical references, and lease the assignment."""
        ...
        for state, reason in (
            (TaskState.INTERPRETING, "framing the required outcome"),
            (TaskState.READY, "no blocker prevents planning"),
        ):
        ...
        # Authority and the assignment are resolved inside AUTHORIZING, and the
        # Task takes the assignment's context manifest as its own single value.
        ...
        if decision is not AuthorityDecision.ALLOW:
            self.tasks.transition(
                ...
                task["revision"], correlation_ref=task_id, blocker_ref=envelope.envelope_id,
            )
        ...
        task = self.tasks.transition(
            task_id, TaskState.EXECUTING, actor, "authorized work is progressing", task["revision"],
            correlation_ref=task_id, active_step_ref=assignment.step_ref,
        )
        self.tasks.record_event(
            task_id, "assignment.leased", worker_id, "worker leased a bounded assignment",
            correlation_ref=task_id,
            payload={"assignment_id": assignment.assignment_id, "fencing_token": lease.fencing_token,
                     "project_boundary": assignment.project_boundary},
        )
        return FlowBinding(
            task_id=task_id, task_revision=task["revision"], assignment_id=assignment.assignment_id,
    ...
    def execute(
        ...
        *,
        ...
        proposed = self.tasks.record_event(
            binding.task_id, "action.proposed", executor, request.intended_effect,
            correlation_ref=binding.correlation_ref,
            payload={"action_request_id": request.action_request_id, "operation": request.operation,
                     "target": request.target, "adapter_id": request.adapter_id},
        )
        try:
            self.actions.authorize(request.action_request_id, envelope, moment)
        except Exception as exc:
            self.tasks.record_event(
                binding.task_id, "action.denied", self.verifier, str(exc),
                correlation_ref=binding.correlation_ref, causal_ref=proposed,
                payload={"action_request_id": request.action_request_id},
            )
            raise
        authorized = self.tasks.record_event(
            binding.task_id, "action.authorized", self.verifier, envelope.reason,
            correlation_ref=binding.correlation_ref, causal_ref=proposed,
            payload={"action_request_id": request.action_request_id, "envelope_id": envelope.envelope_id,
                     "envelope_version": envelope.version},
        )
        attempt = self.actions.dispatch(request.action_request_id, executor, interface_version, adapter)
        self.tasks.record_event(
            binding.task_id, "action.attempted", executor, f"attempt {attempt['status']}",
            correlation_ref=binding.correlation_ref, causal_ref=authorized,
            payload={"action_request_id": request.action_request_id, "attempt_id": attempt["attempt_id"],
                     "status": attempt["status"], "effect_state": attempt["effect_state"]},
        )
        return attempt
    ...
    def reconcile(
        ...
        *,
        ...
        """Resolve an effect against inspected reality, backed by recorded Evidence."""
        moment = now or utc_now()
        self._require_current(binding, moment)
        ...
        self.tasks.record_event(
            binding.task_id, "action.effect_reconciled", self.verifier,
            f"effect resolved to {effect_state}",
            correlation_ref=binding.correlation_ref,
            payload={"action_request_id": request_id, "effect_state": str(effect_state),
                     "evidence_id": evidence.evidence_id, "command_replayed": False},
        )
        return attempt
    ...
    def settle_assignment(
        ...
        *,
        ...
        """Settle the worker's bounded assignment under its current fencing token."""
        moment = now or utc_now()
        self._require_current(binding, moment)
        ...
        result = self.assignments.submit_result(
            binding.lease, result_state, output_ref, refs, tuple(limitations), moment
        )
        self.tasks.record_event(
            binding.task_id, "assignment.settled", binding.lease.worker_id, f"worker reported {result_state}",
            correlation_ref=binding.correlation_ref,
            payload={"assignment_id": binding.assignment_id, "result_state": str(result_state),
                     "result_id": result["result_id"], "evidence_refs": list(refs)},
        )
        return result
    ...
    def complete(
        ...
        *,
        ...
        """Run the IMMUNE SYSTEM completion gate and act only on its verdict.

        A non-verified verdict blocks the Task with the gate's own reason. The Task
        cannot reach COMPLETED by any other path: the manifest is the only source of
        a consumable completion assessment.
        """
        moment = now or utc_now()
        ...
        manifest = self.evidence.evaluate_completion(
            task_ref=binding.task_id, task_revision=binding.task_revision,
            assignment_ref=binding.assignment_id, scope=self._scope(binding),
            required_criteria=required_criteria, assignment_result_state=assignment["state"],
            effects=self._effects(binding.task_id), artifact_refs=artifacts,
            outcome_exists=bool(result and result["output_ref"]), outcome_statement=outcome_statement,
            exclusions=exclusions, verifier=self.verifier, now=moment,
        )
        self.tasks.record_event(
            binding.task_id, "completion.evaluated", self.verifier, manifest.reason,
            correlation_ref=binding.correlation_ref,
            payload={"manifest_id": manifest.manifest_id, "verdict": str(manifest.verdict),
                     "criteria_unsatisfied": list(manifest.criteria_unsatisfied)},
        )
        if not manifest.is_verified():
            self.tasks.transition(
                binding.task_id, TaskState.BLOCKED, self.verifier,
                f"completion gate {manifest.verdict}: {manifest.reason}", binding.task_revision,
                correlation_ref=binding.correlation_ref, blocker_ref=manifest.manifest_id,
                excluded_scope=manifest.exclusions, artifact_refs=manifest.artifact_refs,
            )
            return manifest

        verifying = self.tasks.transition(
            binding.task_id, TaskState.VERIFYING, actor, "evaluating evidence against completion criteria",
            binding.task_revision, correlation_ref=binding.correlation_ref,
        )
        self.tasks.transition(
            binding.task_id, TaskState.COMPLETED, self.verifier, manifest.reason, verifying["revision"],
            correlation_ref=binding.correlation_ref, completion=manifest.to_completion_evidence(),
            evidence_refs=manifest.evidence_refs, artifact_refs=manifest.artifact_refs,
    ...
    def resume(self, task_id: str) -> NextStep:
        """Reconstruct the valid next step from durable state alone.

        Read-only by construction: it dispatches nothing and replays no command.
        An unknown effect dominates every other consideration.
        """
        ...
        for request in self.actions.requests_for_task(task_id):
            ...
            if effect in {EffectState.UNKNOWN, EffectState.POSSIBLE}:
                return NextStep(
                    state, NextAction.RECONCILE_EFFECT,
                    f"action effect is {effect}; reconcile against reality before any retry",
                    request["action_request_id"], assignment["assignment_id"],
                )

        if assignment["state"] in {AssignmentState.READY, AssignmentState.RECOVERING}:
            return NextStep(state, NextAction.CLAIM_ASSIGNMENT,
                            f"assignment is {assignment['state']} and needs a fresh fenced lease",
                            None, assignment["assignment_id"])
        if assignment["state"] == AssignmentState.LEASED:
            executed = [r for r in self.actions.requests_for_task(task_id)
                        if r["state"] in {ActionState.RESULT_RECEIVED, ActionState.EFFECT_RECONCILED, ActionState.VERIFIED}]
            action = NextAction.SETTLE_ASSIGNMENT if executed else NextAction.EXECUTE_ACTION
            reason = ("execution truth exists; settle the assignment"
                      if executed else "the lease is held and no action has produced a result")
            return NextStep(state, action, reason, None, assignment["assignment_id"])

        manifest = self.evidence.latest_manifest(task_id)
        if manifest is None:
            return NextStep(state, NextAction.EVALUATE_COMPLETION,
                            "the assignment is settled and the completion gate has not run",
                            None, assignment["assignment_id"])
        if manifest["verdict"] == "VERIFIED":
            return NextStep(state, NextAction.COMPLETE_TASK,
                            "a verified completion manifest exists and the task is not yet COMPLETED",
                            None, assignment["assignment_id"])
        return NextStep(state, NextAction.NONE,
                        f"completion gate returned {manifest['verdict']}; a new authorized decision is required",
                        None, assignment["assignment_id"])
    ...
    def _require_bindable(self, task_id: str, assignment: SpecialistAssignment, envelope: AuthorityEnvelope) -> None:
        ...
        if outside:
            raise BoundaryViolation(
                "delegated tools exceed the envelope tool boundary: " + ", ".join(sorted(outside))
            )
    ...
    def _require_evidence_scope(self, binding: FlowBinding, items: Iterable[Evidence]) -> None:
        ...
        for item in items:
            if item.scope != scope:
                raise BoundaryViolation(
                    f"evidence {item.evidence_id} is scoped to {item.scope}, not {scope}"
                )

    def _request_effect(self, request: dict) -> EffectState:
        attempt = self.actions.latest_attempt(request["action_request_id"])
        if attempt is None:
            # A DISPATCHED request with no attempt row is a crash between the state
            # change and the attempt commit. Absence of a record is not proof of
            # absence of effect.
    ...
    def _effects(self, task_id: str) -> tuple[EffectRecord, ...]:
        return tuple(
            EffectRecord(request["action_request_id"], str(self._request_effect(request)))
            for request in self.actions.requests_for_task(task_id)
        )
