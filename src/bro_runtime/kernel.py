...
class BROKernel:
    ...
    def __init__(self, task_store: SQLiteTaskStore, mind_store: SQLiteMindStore) -> None:
        ...
        self.perception = PerceptionStore(task_store.connection)
        self.mind = MindRuntime(mind_store)
        self.nervous = NervousRecordStore(task_store.connection)
        self.skills = CapabilityRegistry(task_store.connection)
    ...
    def prepare(
        ...
        """Create the canonical records needed before authority/execution begins."""
        ...
        context = self.nervous.create_context_manifest(
            ...
            entries=(ContextEntry(
                ...
                inclusion_reason="The current user request defines the outcome and scope.",
        ...
        assignment = SpecialistAssignment(
            ...
            authority_envelope_ref="UNBOUND", allowed_tools=tuple(capability.operations),
            deadline=None, budget={}, evidence_requirements=success_conditions,
        )
        return PreparedFlow(intent.intent_id, goal.goal_id, decision.decision_id, plan.plan_id,
                            step.step_id, context.manifest_id, capability.capability_id, assignment)

    def open(self, prepared: PreparedFlow, envelope: AuthorityEnvelope, *, worker_id: str, now: str | None = None):
        """Bind the prepared flow to IMMUNE authority and enter governed execution."""
        if envelope.task_ref != prepared.assignment.task_ref:
            raise KernelRejected("authority envelope belongs to a different Task")
        assignment = SpecialistAssignment(**{
            **prepared.assignment.__dict__,
            "authority_envelope_ref": envelope.envelope_id,
            "allowed_tools": tuple(envelope.tool_boundary),
        })
        return self.supervisor.open_flow(
