...
class KernelIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.kernel.skills.register(Capability(
            capability_id="cap:crm-write",version=1,kind=CapabilityKind.TOOL_ADAPTER,name="CRM writer",description="Writes governed CRM automations",
            operations=("write",),domains=("crm",),input_contract_ref=None,output_contract_ref="artifact:automation",
            dependency_refs=(),authority_requirements=("write",),evidence_capabilities=("inspect",),provider_ref="adapter:crm",health_ref=None,
            status=CapabilityStatus.ACTIVE,recorded_at=T0))

    def prepare(self):
        return self.kernel.prepare(request="Automate lead routing",source="user:gev",project_boundary="BRO",
            desired_outcome="Working lead routing automation",interpreted_scope=("crm","lead-routing"),success_conditions=("automation works",),
            operation="write",domain="crm",authority_basis="user request",materiality="MATERIAL",risk_class="R2",
            expected_output="artifact:automation",verification_requirement="inspect deployed routing")

    def test_missing_capability_fails_before_task_execution(self):
        with self.assertRaises(KernelRejected):
            self.kernel.prepare(request="x",source="user",project_boundary="BRO",desired_outcome="x",interpreted_scope=("x",),success_conditions=("x",),operation="delete",domain="unknown",authority_basis="user",materiality="MATERIAL",risk_class="R2",expected_output="artifact:x",verification_requirement="inspect")
    ...
    def test_outcome_request_reaches_verified_terminal_state(self):
        ...
        envelope=AuthorityEnvelope(envelope_id="auth:1",version=1,principal="BRO",proof_ref="proof:user",authority_source="user",
            operation="write",target="crm:lead-routing",allowed_scope=("operation:write","target:crm:lead-routing",task_id,"project:BRO"),prohibited_scope=(),
            task_ref=task_id,risk_class="R2",valid_from=T0,expires_at="2026-09-02T00:00:00Z",revocation_ref=None,environment="prod",
            tool_boundary=("adapter:crm",),decision="ALLOWED",reason="explicit bounded user request",audit_ref="audit:1")
        binding=self.kernel.open(prepared,envelope,worker_id="specialist:automation",now=T1)
        request=ActionRequest(action_request_id="action:1",task_ref=task_id,intended_effect="deploy lead routing",operation="write",target="crm:lead-routing",
            environment="prod",adapter_id="adapter:crm",input_parameters={"rule":"route"},authority_envelope_ref="auth:1",risk_class="R2",
            reversibility="REVERSIBLE",idempotency_key="lead-routing-v1",idempotency_guaranteed=True,expected_result="deployed",
            verification_requirements=("inspect",),assignment_ref=prepared.assignment.assignment_id,project_boundary="BRO")
        attempt=self.kernel.supervisor.execute(binding,request,executor="specialist:automation",interface_version="1",adapter=lambda _:AdapterResult("deployed",EffectState.CONFIRMED))
